...
### Pitfall 3: Gutter “click” vs decoration API

**What goes wrong:** UI-SPEC / D-12 describe **click** on a gutter glyph → QuickPick or direct open. The stable VS Code API documents `DecorationOptions.range`, `hoverMessage`, and type-level `gutterIconPath` ([VS Code API](https://code.visualstudio.com/api/references/vscode-api)) — **not** a decoration `onClick` callback.  
**Why it happens:** Gutter icons are render-only affordances unless paired with another mechanism.  
**How to avoid:** **Spike early:** e.g. command **`LeanQuill: Issues at cursor`** (or context menu on editor) that resolves the active line to stacked issues and runs `showQuickPick`; optionally add **Markdown command links** in `hoverMessage` as a secondary path. If product requires **literal mouse click on the glyph**, confirm against the **exact** `engines.vscode` baseline (proposed APIs change).  
**Warning signs:** Plan assumes `setDecorations` alone satisfies D-12 without a separate interaction channel.
