...
- **Global scope**: `C:/Dev/LeanQuill/.cursor/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Dev/LeanQuill/.cursor/notes/`)
...
2. Glob `C:/Dev/LeanQuill/.cursor/notes/*.md` (if directory exists) — global notes
...
Global (C:/Dev/LeanQuill/.cursor/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Dev/LeanQuill/.cursor/notes/` — works in any directory
